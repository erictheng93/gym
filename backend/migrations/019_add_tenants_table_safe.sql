-- ============================================
-- 多租户架构 Migration (Safe Version)
-- 版本: 019
-- 日期: 2026-01-07
-- 說明:
--   1. 創建租戶主表 (tenants)
--   2. 如果 branches 表存在，則添加 tenant_id
--   3. 實現租戶隔離和多租戶架構
--   4. 支持套餐管理、配額限制、計費系統
-- ============================================
...
-- ============================================
-- 4. 如果 branches 表存在，更新添加 tenant_id
-- ============================================
DO $$
BEGIN
    -- 檢查 branches 表是否存在
    IF EXISTS (
        ...
        IF NOT EXISTS (
            ...
            RAISE NOTICE 'Added tenant_id column to branches table';
            ...
            IF NOT EXISTS (
                ...
                RAISE NOTICE 'Set tenant_id as NOT NULL';
            END IF;
            ...
            RAISE NOTICE 'Branches table updated with tenant_id successfully';
        ELSE
            RAISE NOTICE 'tenant_id column already exists in branches table';
        END IF;
    ELSE
        RAISE NOTICE 'Branches table does not exist yet - will be added when branches table is created';
    END IF;
END $$;
...
BEGIN
    ...
    INTO v_max_count
    FROM tenants
    WHERE id = p_tenant_id AND tenant_status IN ('active', 'trial');
...
BEGIN
    IF EXISTS (
        ...
        SELECT
            ...
            -- 當前使用量（如果相關表存在則計算，否則為 0）
            COALESCE(COUNT(DISTINCT b.id), 0) AS current_branches,
            COALESCE(COUNT(DISTINCT m.id), 0) AS current_members,
            COALESCE(COUNT(DISTINCT e.id), 0) AS current_employees,

            -- 配額使用率
            CASE
                WHEN t.max_branches > 0 THEN
                    ROUND(COUNT(DISTINCT b.id)::NUMERIC / NULLIF(t.max_branches, 0) * 100, 2)
                ELSE 0
            END AS branches_usage_percent,
            CASE
                WHEN t.max_members > 0 THEN
                    ROUND(COUNT(DISTINCT m.id)::NUMERIC / NULLIF(t.max_members, 0) * 100, 2)
                ELSE 0
            END AS members_usage_percent,
            CASE
                WHEN t.max_employees > 0 THEN
                    ROUND(COUNT(DISTINCT e.id)::NUMERIC / NULLIF(t.max_employees, 0) * 100, 2)
                ELSE 0
            END AS employees_usage_percent,

            -- 活躍合約數
            COALESCE(COUNT(DISTINCT c.id) FILTER (WHERE c.status = ''active''), 0) AS active_contracts
        ...
        RAISE NOTICE 'Created v_tenant_overview view successfully';
    ELSE
        RAISE NOTICE 'Skipping v_tenant_overview view creation - branches table does not exist';
    END IF;
END $$;
...
-- 顯示創建結果
SELECT 'Tenants table migration completed successfully!' AS result;
