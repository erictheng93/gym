...
const currentEmployee = ref<{ id: string; full_name: string; supervisor_id?: string } | null>(null)
  ...
  try {
    ...
    const employees = await directus.request(
      readItems('employees', {
        ...
        fields: ['id', 'full_name', 'supervisor_id'],
        limit: 1
      })
    )

    if (employees.length > 0) {
      currentEmployee.value = employees[0] as typeof currentEmployee.value
      await loadData()
    }
  } catch (error) {
...
const getStatusBadge = (status: string) => {
  const map: Record<string, { label: string; class: string }> = {
    PENDING: { label: PAGES.HR.LEAVES.PENDING_APPROVAL, class: 'badge-warning' },
    APPROVED: { label: PAGES.HR.LEAVES.APPROVED, class: 'badge-success' },
    REJECTED: { label: PAGES.HR.LEAVES.REJECTED, class: 'badge-error' },
    CANCELLED: { label: PAGES.HR.LEAVES.CANCELLED, class: 'badge-secondary' }
  }
...
const handleApply = async () => {
  if (!currentEmployee.value || isSubmitting.value) return
...
// 審核
const handleReview = async (leaveId: string, action: 'APPROVE' | 'REJECT') => {
  if (!currentEmployee.value) return

  const actionLabel = action === 'APPROVE' ? '核准' : '駁回'
  const notes = prompt(`請輸入${actionLabel}備註（選填）：`)

  try {
    await reviewLeave(leaveId, currentEmployee.value.id, action, notes || undefined)
    await loadData()
  } catch (error) {
...
<template>
  <div class="leaves-page">
    ...
    <div v-if="activeTab === 'my-leaves'" class="tab-content">
      ...
      <!-- Leave Requests List -->
      <div v-if="isLeavesLoading" class="loading-state">
      ...
      <div v-else-if="leaveRequests.length === 0" class="empty-state">
        ...
        <p>{{ PAGES.HR.LEAVES.NO_LEAVES }}</p>
        <button class="btn btn-primary mt-lg" @click="showApplyModal = true">
          {{ PAGES.HR.LEAVES.APPLY_LEAVE }}
        </button>
      </div>
      ...
      <div v-else class="leaves-list">
        ...
        >
          <div class="leave-header">
            ...
            <span :class="['badge', getStatusBadge(leave.leave_status).class]">
              {{ getStatusBadge(leave.leave_status).label }}
            </span>
          </div>
      ...
      <div v-else class="leaves-list">
        ...
        >
          ...
          <div class="approval-actions">
            <button class="btn btn-success" @click="handleReview(leave.id, 'APPROVE')">
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <polyline points="20 6 9 17 4 12"/>
              </svg>
              {{ PAGES.HR.LEAVES.APPROVE }}
            </button>
            <button class="btn btn-error-outline" @click="handleReview(leave.id, 'REJECT')">
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
