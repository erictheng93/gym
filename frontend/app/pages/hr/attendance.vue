...
const currentEmployee = ref<{ id: string; branch_id: string; full_name: string } | null>(null)
...
const handleCheckIn = async () => {
  if (!currentEmployee.value || isProcessing.value) return
...
const getStatusBadge = (status: string) => {
  const map: Record<string, { label: string; class: string }> = {
    PRESENT: { label: PAGES.HR.ATTENDANCE.PRESENT, class: 'badge-success' },
    LATE: { label: PAGES.HR.ATTENDANCE.LATE, class: 'badge-warning' },
    EARLY_LEAVE: { label: PAGES.HR.ATTENDANCE.EARLY_LEAVE, class: 'badge-warning' },
    ABSENT: { label: PAGES.HR.ATTENDANCE.ABSENT, class: 'badge-error' },
    LEAVE: { label: PAGES.HR.ATTENDANCE.ON_LEAVE, class: 'badge-info' }
  }
...
const currentBranchName = computed(() => {
  if (!currentEmployee.value?.branch_id) return ''
  return branches.value.find(b => b.id === currentEmployee.value?.branch_id)?.name || ''
})
</script>
...
<template>
  <div class="attendance-page">
    ...
    <section class="records-section">
      ...
      <div v-if="isAttendanceLoading" class="loading-state">
        <div class="loading-spinner"></div>
      </div>

      <div v-else-if="recentAttendances.length === 0" class="empty-state">
        <p class="text-secondary">{{ PAGES.HR.ATTENDANCE.NO_RECORDS }}</p>
      </div>
      ...
      <div v-else class="records-list">
        ...
        >
          <div class="record-date">
            ...
            <span :class="['badge', getStatusBadge(record.attendance_status).class]">
              {{ getStatusBadge(record.attendance_status).label }}
            </span>
          </div>
