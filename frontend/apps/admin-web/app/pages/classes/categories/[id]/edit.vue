...
const { getCategory, updateCategory, fetchRootCategories } = useClassCategories()
...
const loadCategory = async () => {
  ...
  try {
    ...
    form.value = {
      ...
      owner_branch_id: data.owner_branch_id
    }
  } catch (error) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    toast.error(MESSAGES.ERRORS.CATEGORY_UPDATE_FAILED)
  } finally {
...
onMounted(async () => {
  try {
    // Load root categories excluding current one (to prevent circular reference)
    const allRoots = await fetchRootCategories()
    rootCategories.value = allRoots.filter(c => c.id !== categoryId.value)
    await fetchBranches()
    await loadCategory()
  } catch (error) {
...
<template>
  <PageContainer>
    <!-- Header -->
    <PageHeader
      :title="PAGES.CLASS_CATEGORIES.EDIT_CATEGORY"
    ...
    <!-- Form -->
    <form v-else class="category-form card" @submit.prevent="handleSubmit">
      ...
      <div class="form-actions">
        ...
        >
          <span v-if="isSubmitting">{{ MESSAGES.ACTIONS.UPDATING }}</span>
          <span v-else>{{ MESSAGES.FORM.SAVE_CHANGES }}</span>
        </button>
      </div>
    </form>
  </PageContainer>
